...
import { Body, AccountUpdate, signJsonTransaction, Permissions, ZkappPublicInput, Events, SequenceEvents, accountUpdateToPublicInput, Authorization, CallForest, TokenId, smartContractContext, } from './account_update.js';
...
// external API
export { SmartContract, method, deploy, signFeePayer, declareMethods, Callback, Account, };
// internal API
    ...
    return function wrappedMethod(...actualArgs) {
                ...
                if (inCheckedComputation() && !context.isCallback) {
                    ...
                    let [, result] = Mina.currentTransaction.runWith({
                        ...
                        if (DEBUG_PUBLIC_INPUT_CHECK) {
                            ...
                            Circuit.asProver(() => {
                                let inputAccountUpdate = snarkContext.get().proverData;
                                diff(accountUpdate.toJSON(), inputAccountUpdate.toJSON());
                                let nChildren = inputAccountUpdate.children.accountUpdates.length;
                                for (let i = 0; i < nChildren; i++) {
                                    let inputChild = inputAccountUpdate.children.accountUpdates[i].toJSON();
                                    let child = accountUpdate.children.accountUpdates[i].toJSON();
        ...
        let [, result] = smartContractContext.runWith({
            ...
            let { accountUpdate, result } = methodCallDepth === 0
                ? AccountUpdate.witness(returnType ?? circuitValue(null), runCalledContract, { skipCheck: true })
                : runCalledContract();
            ...
            accountUpdate.children.calls = Circuit.witness(Field, () => CallForest.hashChildren(accountUpdate));
...
class Callback extends GenericArgument {
    constructor(instance, methodName, args) {
        ...
        methodIntf = {
            ...methodIntf,
            returnType: methodIntf.returnType ??
                circuitValue(null),
        };
    ...
    static create(instance, methodName, args) {
        let callback = new this(instance, methodName, args);
        return callback;
    }
}
// TODO: prove call signature in the outer circuit, just like for composability!
function accountUpdateFromCallback(parentZkapp, childLayout, callback) {
    let { accountUpdate } = AccountUpdate.witnessTree(circuitValue(null), childLayout, () => {
        if (callback.isEmpty)
...
function selfAccountUpdate(address, tokenId) {
    let body = Body.keepAll(address);
    if (tokenId) {
        body.tokenId = tokenId;
        body.caller = tokenId;
    }
    return new AccountUpdate(body, {}, true);
}
