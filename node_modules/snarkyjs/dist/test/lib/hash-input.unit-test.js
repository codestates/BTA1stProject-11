import { isReady, AccountUpdate, PrivateKey, Types, Field, Ledger, UInt64, UInt32, Bool, Permissions, Sign, Token, shutdown, } from '../index.js';
...
// timing
let Timing = asFieldsAndAux(jsLayout.AccountUpdate.entries.body.entries.update.entries.timing.inner);
...
// permissions
let Permissions_ = asFieldsAndAux(jsLayout.AccountUpdate.entries.body.entries.update.entries.permissions.inner);
...
testInput(Permissions_, Ledger.hashInputFromJson.permissions, permissions.value);
// update
...
let AccountPrecondition = asFieldsAndAux(jsLayout.AccountUpdate.entries.body.entries.preconditions.entries.account);
...
testInput(AccountPrecondition, Ledger.hashInputFromJson.accountPrecondition, account);
// network precondition
let NetworkPrecondition = asFieldsAndAux(jsLayout.AccountUpdate.entries.body.entries.preconditions.entries.network);
...
testInput(NetworkPrecondition, Ledger.hashInputFromJson.networkPrecondition, network);
// body
...
testInput(Types.AccountUpdate, (accountUpdateJson) => Ledger.hashInputFromJson.body(JSON.stringify(JSON.parse(accountUpdateJson).body)), accountUpdate);
...
function testInput(Module, toInputOcaml, value) {
    ...
    expect(JSON.parse(JSON.stringify(fields1))).toEqual(JSON.parse(JSON.stringify(fields2)));
